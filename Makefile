...
# Reformat code style
format:
	$(poetry_exec) run black advanced_aiocontext && \
	git add -u && \
	$(poetry_exec) run isort advanced_aiocontext && \
	git add -u && \
	$(poetry_exec) run autoflake \
		--ignore-init-module-imports \
		--remove-unused-variables \
		--recursive \
		--in-place advanced_aiocontext tests

# Run tests locally
test:
	$(poetry_exec) run pytest tests --cov=advanced_aiocontext --cov-report=html

# Tests command for CI with .coveragerc report
test-ci:
	$(poetry_exec) run coverage run --source=advanced_aiocontext -m pytest tests
